...
export const TicketPanelComp = ({ ticketId }) => {
    ...
    const handleResponseSubmit = async () => {
        if (responseCtrl != "") {
            await axios.post(apiUrl + "sendResponse", { idTicket: ticketId, jwt: jwt, content: responseCtrl })
        ...
        ticketInfos.length > 0 ? (
            <main className='container_ticketpanel_page '>
                ...
                <section className='columnContainer container_panel_btn'>
                    ...
                    <textarea className='textarea_response' onChange={(e) => setResponseCtrl(e.target.value)} placeholder='Entrez une réponse' name="response" cols="30" rows="10" />
                    <button onClick={() => handleResponseSubmit} className='btn_ticket_panel btn_response_ticket'>Répondre</button>
                </section>
            </main >)
